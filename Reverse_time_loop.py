...
@jit(nopython=True)            
def rt_update_vw(xl,zl,dx,dz,dt,rho,vp,CPML,a_x,a_z,b_x,b_z,k_x,k_z,v,w,p,memory_dp_dx,memory_dp_dz):
    c1=1.23404
    c2=-0.10665
    c3=0.0230364
    c4=-0.00534239
    ...
    for j in range(8,z_len-8):
        for i in range(8,x_len-8):
            ...
            if (i>=CPML+8) and (i<x_len-CPML-8) and (j>=CPML+8) and (j<z_len-CPML-8):
                p[i][j]+=k_value[i][j]/rho[i][j]*(value_dv_dx+value_dw_dz)*dt
...
def reverse_time_loop(xl,zl,dx,dz,dt,rho,vp,CPML_Params,k_max,ref_pos,rhs_data):
    ...
    for tt in range(k_max-1,-1,-1):   
        p[ref_pos[:,0],ref_pos[:,1]]+=rhs_data[tt]*rho[ref_pos[:,0],ref_pos[:,1]]*vp[ref_pos[:,0],ref_pos[:,1]]**2
        # p[ref_pos[:,0],ref_pos[:,1]]+=rhs_data[tt]
        v,w=rt_update_vw(xl,zl,dx,dz,dt,rho,vp,CPML,a_x_half,a_z_half,b_x_half,b_z_half,k_x_half,k_z_half,v,w,p,memory_dp_dx,memory_dp_dz)
        p=rt_update_p(xl,zl,dx,dz,dt,rho,vp,CPML,a_x,a_z,b_x,b_z,k_x,k_z,v,w,p,memory_dv_dx,memory_dw_dz)
        yield p.tolist(),v.tolist(),w.tolist()
